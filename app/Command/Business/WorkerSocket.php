...
class WorkerSocket
{
    ...
    protected function repair(): void
    {
        ...
        $this->logger->error(sprintf('Repair socket %s:%s connect starting', $this->host, $this->port));
        ...
        while ($this->running === true) {
            try {
                ...
                if ($this->closed === true) {
                    $this->logger->notice(sprintf('Repair socket %s:%s connect ok', $this->host, $this->port));
                    $this->socket = $socket;
                    break;
                }
                if ($socket->send($data) !== false) {
                    $this->socket = $socket;
                    $this->logger->notice(sprintf('Repair socket %s:%s connect and register ok', $this->host, $this->port));
                    break;
                }
                $socket->close();
                $this->logger->error(sprintf('Repair socket %s:%s connect failed, wait for three seconds before continuing', $this->host, $this->port));
                Coroutine::sleep(3);
            } catch (Throwable) {
            }
        }
    ...
    public function connect(): void
    {
        ...
        $this->socket = $socket;
        $this->logger->notice(sprintf('Socket %s:%s connect ok', $this->host, $this->port));
    }
    ...
    public function close(): void
    {
        ...
        while ($this->running) {
            Coroutine::sleep(1);
            $this->logger->debug(sprintf('Closing socket %s:%s, Wait for sendCh cleaning completed: %d', $this->host, $this->port, $this->sendCh->length()));
    ...
    public function loopHeartbeat(): void
    {
        ...
        Coroutine::create(function () {
            ...
            $this->logger->debug('Coroutine:loopHeartbeat exit');
        });
    }
    ...
    public function loopSend(): void
    {
        ...
        Coroutine::create(function () {
            ...
            $this->logger->debug('Coroutine:loopSend exit');
        });
    }
